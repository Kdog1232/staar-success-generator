...
- For EACH question include:
  - 🧠 Strategy: one short line naming the thinking move before solving (example: Main Idea = what the passage is mostly about overall).
  - 3-step tutor flow that guides thinking BEFORE the answer:
    1) What is the question really asking?
    2) What evidence should I look for?
    3) Eliminate wrong answers with brief option-by-option reasoning.
...
- Keep tone like a live tutor guiding one child through one exact question.
- Never jump straight to the final answer; show the reasoning path first.` : `- tutorMode is false. Do not include a TUTOR MODE section.`}
