...
serve(async (req) => {
  ...
  const returnCore = async (data: CoreResponse) => {
    ...
    while (practice.questions.length < 5) {
      practice.questions.push({ ...practice.questions[practice.questions.length - 1] });
    }
    console.warn("⚠️ Using minimally repaired AI output");
  ...
  const returnEnrichment = (data: WorkerAttempt) =>
    {
      ...
      while (practiceQuestions.length < 5) {
        practiceQuestions.push({ ...practiceQuestions[practiceQuestions.length - 1] });
      }
      ...
      try {
        if (effectiveMode === "core") {
            ...
            while (coreQuestions.length < 5) {
              coreQuestions.push({ ...coreQuestions[coreQuestions.length - 1] });
            }
        ...
        while (safePracticeQuestions.length < 5) {
          safePracticeQuestions.push({ ...safePracticeQuestions[safePracticeQuestions.length - 1] });
        }
